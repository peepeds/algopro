...
#define R "\x1b[0m"
#define O "\x1b[30;107m" // Background Putih 
...
void intro(){ CLR;  // user bisa milih mau ke menu yg mana 
    
    art(); // nampilin art design 
    p (23, -5, W"[Sign Up]"); 
    p (-15,-5, "[Login]");				 		 
    p (4,  -5, "[Credits]");				 		 
    p (3,  -8, "[Exit.]");				 		 
    ...
    intro_control(fns); // pergi ke pilihan user 
...
void lobby(){ CLR;
    ...
    lobby_control(fns); // pergi ke  pilihan user 

}

void menu(){ CLR; // nampilin & milih list menu yg ada 
...
void chinese(){ //daftar menu chinese
...
void western(){ // daftar menu western 
...
void exits(){
    ...
    exit(0); // berehentiin progeam 

}

void credits(){ CLR; // nampilin credit doang 
...
int check_hp(char *hp){ // buat check ke validan nomer hp di indonesia 
    ...
    else valid = 0;  // berarti ga valid 
    return valid;

}

int check_mail(char *email){ // ccheck kevalidan email 

    int len = strlen(email), akeong = 0, username_len = 0, domain_len = 0, titik = 0, tld = 0;
    
    for(int i=0 ;i<len; i++){
        if(email[i] == '@'){ // harus ada @ di email 
            akeong = 1;
            break;
        }
    }
    ...
    // di break soalnya infinite loop 

    if(len>100) // kepanjangan
        return 0; 
    
    if(!akeong) // kalau  ga ada '@'
        return 0;
        
    if(username_len==0)  // kalau sebelum @ ga ada char
        return 0;
        
    if(domain_len<5 || !titik || !tld)  // kalau sesesudah @ panjangnya dibawah 5 , ga ada '.' , ga ada domain.
        return 0;
...
int check_login(char *email, char* password){ // check email & password tersedia ga 
	...
	while(fgets(line, 200, fp)!=NULL){ // bandingin email & pass yang dia tulis sama  yg ada di db 
        sscanf(line,"%s %s", curr_mail, curr_pass);
        if(strcmp(email, curr_mail)==0&&strcmp(password,curr_pass)==0){ // kalau sama berarti ketemu 
            check = 1; // artinya ketemu 
            break;  // loop diberhentiin 
        }
    }
...
void signup(){ CLR; // skenario user bila belum memiliki akun 
	
    while(1){ CLR; // bikin akun 
        ...
        if(!check_mail(email)){ // harus sesuai kriteria diatas 
		...
		if(check_email){ // check kevalidan email 
	    ...
	    int len=0, lower=0, upper=0, number=0; // kriteria kevalidan pass 
		...
		scanf("%[^\n]",validasi); getchar(); // minta user confirm password

		if(strcmp(password,validasi)){ // kalo ga sama 
...
void login(){ CLR;
    ...
    while(1){ CLR; 
        ...
        if(!check_login(login_email, login_password)){ // kalo email & pw ga match sama di db 
